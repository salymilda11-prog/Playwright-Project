...
test(`locators ${testdata.productitem}` , async({page})=>{ //concat method
    ...
    await pi.productdetails(testdata.productitem)
    await pi.gotocart()
    await sc.shoppingcart()
    await ck.gotocheckout(testdata.firstname, testdata.lastname, testdata.zip)
})
}
